...
class TestCase(flask_testing.TestCase):
    ...
    def create_ip_usage(self, ip='203.0.113.1', project_id=PROJECT_ID,
                        port_id=PORT_ID, resource_id=RESOURCE_ID,
                        resource_type='instance', start=START, end=None):

        ip_usage = models.IPUsage(ip=ip, project_id=project_id,
                                  port_id=port_id, resource_id=resource_id,
                                  resource_type=resource_type, start=start,
                                  end=end)
